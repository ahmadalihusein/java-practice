// Exercise 01 — Hello + variables
// RUN IT:  java 01-basics\Exercise01_Hello.java
// This one is already done so you can confirm Java works. Read it, run it.

public class Exercise01_Hello {
    public static void main(String[] args) {
        String name = "future intern";
        int year = 2026;
        System.out.println("Hello, " + name + "!");
        System.out.println("You will get a job in " + year + ".");

        // TODO: make a variable called `city` set to "Kuala Lumpur"
        //       and print: "Working in Kuala Lumpur"
        // (delete this comment block when done)
    }
}
